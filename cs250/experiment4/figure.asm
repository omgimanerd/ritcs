...
# Name:        block_make
#
# Description: Initialize all the compoenents of the block object.
#              Does essentially the same thing as circle_make
block_make:
...
block_area:
        lw      $t0, 4($a0)    # load the figure width
        lw      $t1, 8($a0)    # load the figure height
        mul     $v0, $t0, $t1  # calculate the area
...
# Note that this does not involve any polymorphic functions.
        ...
        li      $v0, PRINT_INT
        lw      $a0, 4($s0)             # print the figure width
        syscall
...
# Note that this does not involve any polymorphic functions.
        ...
        li      $v0, PRINT_INT
        lw      $a0, 8($s0)             # print the figure height
        syscall
...
# the address of the figure object, and shouldn't be changed.
        ...
        lw      $t0, 0($s0)             # load the figure function table
        lw      $t1, 0($t0)             # load the area function addr
        move    $a0, $s0
        jalr    $t1                     # invoke the area function
...
# contains the address of the figure object, and shouldn't be changed.
        ...
        lw      $t0, 0($s0)             # load the figure function table
        lw      $t1, 4($t0)             # load the perimeter function addr
        move    $a0, $s0
        jalr    $t1                     # invoke the perimeter function
